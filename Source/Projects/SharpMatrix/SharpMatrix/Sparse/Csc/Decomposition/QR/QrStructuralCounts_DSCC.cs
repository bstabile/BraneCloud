 ...
 */
    public class QrStructuralCounts_DSCC
    {
        ...
        internal int[] pinv = new int[0]; // inverse permutation to ensure diagonal elements are all structurally nonzero
        ...
        internal int nz_in_V; // number of entries in V
        internal int nz_in_R; // number of entries in R
        ...
        void countNonZeroUsingLinkedList(int[] parent, int[] ll)
        {
            ...
            if (nz_in_V < 0)
                throw new InvalidOperationException("Too many elements. Numerical overflow in V counts");
        }
