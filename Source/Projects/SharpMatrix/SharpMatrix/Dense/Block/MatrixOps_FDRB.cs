 ...
 */
    public class MatrixOps_FDRB
    {
            ...
            float[] data, float[] tmp)
        {
            ...
            {
                throw new ArgumentException("tmp must be at least " + minLength + " long ");
            }
            ...
            for (int i = 0; i < numRows; i += blockLength)
            {
                ...
                Array.Copy(data, i * numCols, tmp, 0, blockHeight * numCols);
                ...
                for (int j = 0; j < numCols; j += blockLength)
                {
                    ...
                    {
                        Array.Copy(tmp, indexSrcRow, data, indexDst, blockWidth);
                        indexDst += blockWidth;
                        indexSrcRow += numCols;
                    }
                }
            ...
            float[] data, float[] tmp)
        {
            ...
            if (tmp.Length < minLength)
            {
                throw new ArgumentException("tmp must be at least " + minLength + " long and not " + tmp.Length);
            }
            ...
            for (int i = 0; i < numRows; i += blockLength)
            {
                ...
                for (int j = 0; j < numCols; j += blockLength)
                {
                    ...
                    for (int k = 0; k < blockHeight; k++)
                    {
                        Array.Copy(tmp, indexSrc, data, indexDstRow, blockWidth);
                        indexSrc += blockWidth;
                        indexDstRow += numCols;
                    }
                }
        ...
        public static bool isEquals(FMatrixRBlock A, FMatrixRBlock B)
        {
         ...
         */
        public static bool blockAligned(int blockLength, FSubmatrixD1 A)
        {
