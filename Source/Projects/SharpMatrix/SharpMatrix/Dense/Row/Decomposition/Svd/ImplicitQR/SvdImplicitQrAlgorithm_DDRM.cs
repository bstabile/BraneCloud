 ...
 */
    public class SvdImplicitQrAlgorithm_DDRM
    {
        ...
        private readonly int exceptionalThresh = 15;

        private int maxIterations = 15 * 100; // exceptionalThresh*100;
...
//    SimpleMatrix B;
        ...
        public SvdImplicitQrAlgorithm_DDRM(bool fastValues)
        {
        ...
        {
            if (N > M)
                throw new InvalidOperationException("Must be a square or tall matrix");
         ...
         * @return
         */
        public bool process(double[] values)
        {
        ...
        public bool _process()
        {
            ...
            while (x2 >= 0)
            {
                ...
                if (x1 == x2)
                {
//                Console.WriteLine("steps = "+steps+"  script = "+followScript+" at "+x1);
...
//            printMatrix();
        ...
        public bool isDiagonalZero(int i)
        {
//        return Math.Abs(diag[i]) <= maxValue* UtilEjml.EPS;
         ...
         */
        public void performImplicitSingleStep(double scale, double lambda, bool byAngle)
        {
...
//        return Math.Max(scale,Math.Abs(b22));
         ...
         */
        protected void createBulge(int x1, double p, double scale, bool byAngle)
        {
            double b11 = diag[x1];
            double b12 = off[x1];
            double b22 = diag[x1 + 1];
...
//        s = run/gamma;
        ...
        protected void removeBulgeLeft(int x1, bool notLast)
        {
            double b11 = diag[x1];
            double b12 = off[x1];
            double b22 = diag[x1 + 1];
...
//            Console.WriteLine();
        ...
        /// <summary>
        /// Java has a Math.signum method that returns [-1 | 0 | 1].
        /// This does the same thing for value [lt | eq | gt] zero.
        /// </summary>
        private double Signum(double v)
        {
            return v.Equals(0) ? 0 : v < 0.0 ? -1 : 1;
        }
         ...
         */
        protected bool checkForAndHandleZeros()
        {
            ...
            for (int i = x2 - 1; i >= x1; i--)
            {
                if (isOffZero(i))
                {
//                Console.WriteLine("steps at split = "+steps);
                    resetSteps();
                    splits[numSplits++] = i;
                    x1 = i + 1;
                    return true;
...
//            Console.WriteLine();
        ...
        public void setFastValues(bool b)
        {
            fastValues = b;
        }
