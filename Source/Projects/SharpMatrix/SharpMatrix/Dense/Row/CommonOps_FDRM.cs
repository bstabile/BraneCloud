    ...
    public class CommonOps_FDRM
    {
        ...
        public static float dot(FMatrixD1 a, FMatrixD1 b)
        {
            if (!MatrixFeatures_FDRM.isVector(a) || !MatrixFeatures_FDRM.isVector(b))
                throw new InvalidOperationException("Both inputs must be vectors");
        ...
        public static void multInner(FMatrix1Row a, FMatrix1Row c)
        {
            if (a.numCols != c.numCols || a.numCols != c.numRows)
                throw new ArgumentException("Rows and columns of 'c' must be the same as the columns in 'a'");
        ...
        public static void multOuter(FMatrix1Row a, FMatrix1Row c)
        {
            if (a.numRows != c.numCols || a.numRows != c.numRows)
                throw new ArgumentException("Rows and columns of 'c' must be the same as the rows in 'a'");
         ...
         */
        public static bool solve(FMatrixRMaj a, FMatrixRMaj b, FMatrixRMaj x)
        {
         ...
         * @return true if it could invert the matrix false if it could not.
         */
        public static bool invert(FMatrixRMaj mat)
        {
         ...
         * @return true if it could invert the matrix false if it could not.
         */
        public static bool invert(FMatrixRMaj mat, FMatrixRMaj result)
        {
            if (mat.numCols <= UnrolledInverseFromMinor_FDRM.MAX)
            {
                if (mat.numCols != mat.numRows)
        ...
        public static void setIdentity(FMatrix1Row mat)
        {
            ...
            Array.Clear(mat.data, 0, mat.getNumElements());
        ...
        public static void kron(FMatrixRMaj A, FMatrixRMaj B, FMatrixRMaj C)
        {
            ...
            {
                throw new ArgumentException("C does not have the expected dimensions");
            }
            ...
            int dstY0, int dstX0)
        {
            ...
            if (src is FMatrixRMaj && dst is FMatrixRMaj)
            {
            ...
            int[] cols, int colsSize, FMatrixRMaj dst)
        {
            if (rowsSize != dst.numRows || colsSize != dst.numCols)
                throw new ArgumentException("Unexpected number of rows and/or columns in dst matrix");
        ...
        public static void extract(FMatrixRMaj src, int[] indexes, int length, FMatrixRMaj dst)
        {
            if (!MatrixFeatures_FDRM.isVector(dst))
                throw new ArgumentException("Dst must be a vector");
            if (length != dst.getNumElements())
                throw new ArgumentException("Unexpected number of elements in dst vector");
        ...
        public static void extractDiag(FMatrixRMaj src, FMatrixRMaj dst)
        {
            ...
            if (!MatrixFeatures_FDRM.isVector(dst))
            {
                throw new ArgumentException("Expected a vector for dst.");
            }
            else if (dst.getNumElements() != N)
            {
                throw new ArgumentException("Expected " + N + " elements in dst.");
            }
         ...
         */
        public static FMatrixRMaj extractRow(FMatrixRMaj a, int row, FMatrixRMaj output)
        {
            if (output == null)
                output = new FMatrixRMaj(1, a.numCols);
            else if (!MatrixFeatures_FDRM.isVector(output) || output.getNumElements() != a.numCols)
                throw new ArgumentException("Output must be a vector of length " + a.numCols);

            Array.Copy(a.data, a.getIndex(row, 0), output.data, 0, a.numCols);
         ...
         */
        public static FMatrixRMaj extractColumn(FMatrixRMaj a, int column, FMatrixRMaj output)
        {
            if (output == null)
                output = new FMatrixRMaj(a.numRows, 1);
            else if (!MatrixFeatures_FDRM.isVector(output) || output.getNumElements() != a.numRows)
                throw new ArgumentException("Output must be a vector of length " + a.numRows);

            int index = column;
            for (int i = 0; i < a.numRows; i++, index += a.numCols)
            {
                output.data[i] = a.data[index];
            }
            return output;
        }
        ...
        public static void elementMult(FMatrixD1 a, FMatrixD1 b)
        {
            ...
            {
                throw new ArgumentException("The 'a' and 'b' matrices do not have compatible dimensions");
            }
        ...
        public static void elementPower(FMatrixD1 A, FMatrixD1 B, FMatrixD1 C)
        {
            ...
            {
                throw new ArgumentException("All matrices must be the same shape");
            }
        ...
        public static FMatrixRMaj sumRows(FMatrixRMaj input, FMatrixRMaj output)
        {
            if (output == null)
            {
                ...
                throw new ArgumentException("Output does not have enough elements to store the results");
        ...
        public static void add(FMatrixD1 a, float val, FMatrixD1 c)
        {
            if (a.numRows != c.numRows || a.numCols != c.numCols)
            {
                throw new ArgumentException("Dimensions of a and c do not match.");
            }
        ...
        {
            for (var i = 0; i < a.getNumElements(); i++)
                a.data[i] = value;
        }
        ...
        public static FMatrixRMaj rref(FMatrixRMaj A, int numUnknowns, FMatrixRMaj reduced)
        {
            if (reduced == null)
            {
                ...
                throw new ArgumentException("'re' must have the same shape as the original input matrix");
        ...
        public static FMatrixRMaj elements(FMatrixRMaj A, BMatrixRMaj marked, FMatrixRMaj output)
        {
            if (A.numRows != marked.numRows || A.numCols != marked.numCols)
                throw new ArgumentException("Input matrices must have the same shape");
            if (output == null)
         ...
         */
        public static FMatrixRMaj concatColumns(params FMatrixRMaj[] m)
        {
            int rows = 0;
            int cols = 0;
         ...
         * @return Resulting matrix
         */
        public static FMatrixRMaj concatRows(params FMatrixRMaj[] m)
        {
            int rows = 0;
            int cols = 0;
        ...
        public static FMatrixRMaj permuteRowInv(int[] pinv, FMatrixRMaj input, FMatrixRMaj output)
        {
            ...
            for (int row = 0; row < input.numRows; row++)
            {
                Array.Copy(input.data, row * m, output.data, pinv[row] * m, m);
            }
            return output;
        }
    }
