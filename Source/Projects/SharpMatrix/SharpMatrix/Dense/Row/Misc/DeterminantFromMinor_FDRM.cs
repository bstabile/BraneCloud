 ...
 * </p>
 *
 * @see org.ejml.dense.row.decomposition.lu.LUDecompositionAlt_FDRM
 *
 * @author Peter Abeles
 */
    public class DeterminantFromMinor_FDRM
    {
        ...
        public DeterminantFromMinor_FDRM(int width, int minWidth)
        {
            ...
            {
                throw new ArgumentException("No direct function for that width");
            }
...
//        Array.Copy(mat.data,0,minorMatrix[0],0,mat.data.Length);
        ...
        private int openRemove(int where)
        {
            ...
            Array.Copy(open, where + 1, open, where, (numOpen - where - 1));
            numOpen--;
