 ...
 */
    ...
    public class SimpleMatrix<T> : SimpleBase<T>
        where T : class, Matrix
    {
        ...
        {
            setMatrix(new DMatrixRMaj(numRows, numCols, rowMajor, data) as T);
        }

        public SimpleMatrix(int numRows, int numCols, bool rowMajor, float[] data)
        {
            setMatrix(new FMatrixRMaj(numRows, numCols, rowMajor, data) as T);
        }
        ...
        {
            setMatrix(new DMatrixRMaj(data) as T);
        }
        ...
        protected SimpleMatrix(int numRows, int numCols, MatrixType type)
        {
        ...
        public static SimpleMatrix<T> random32(int numRows, int numCols, float minValue, float maxValue, IMersenneTwister rand)
        {
            SimpleMatrix<T> ret = new SimpleMatrix<T>(numRows, numCols, typeof(FMatrixRMaj));
            RandomMatrices_FDRM.fillUniform(ret.mat as FMatrixRMaj, minValue, maxValue, rand);
            return ret;
        }
        ...
        /// <param name="m"></param>
        /// <returns></returns>
        protected override SimpleBase<T> wrapMatrix(T m)
        {
        ...
        /// </summary>
        protected override SimpleBase<T> createMatrix(int numRows, int numCols, MatrixType type)
        {
            return new SimpleMatrix<T>(numRows, numCols, type);
        }
