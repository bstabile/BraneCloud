...
namespace BraneCloud.Evolution.EC.App.Edge.Test
{
    ...
    public class Edge : GPProblem, ISimpleProblem
    {
        ...
        public bool[][] Slurp(FileInfo f) // throws IOException
        {
            var r =
                new StreamReader(new GZipStream(new FileStream(f.FullName, FileMode.Open, FileAccess.Read, FileShare.Read),
        ...
        public override void Setup(IEvolutionState state, IParameter paramBase)
        {
            ...
            if (Generalize)
            {
                if (!ap.Exists) state.Output.Error("File cannot be read", paramBase.Push(P_ALLPOS));
                if (!an.Exists /*.CanRead()*/) state.Output.Error("File cannot be read", paramBase.Push(P_ALLNEG));
            }

            if (!tp.Exists) state.Output.Error("File cannot be read", paramBase.Push(P_TESTPOS));
            if (!tn.Exists) state.Output.Error("File cannot be read", paramBase.Push(P_TESTNEG));
            state.Output.ExitIfErrors();
