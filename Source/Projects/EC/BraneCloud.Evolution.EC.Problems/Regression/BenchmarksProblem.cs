...
namespace BraneCloud.Evolution.EC.Problems.Regression
{
    ...
    ///
    /// </summary>
    public class BenchmarksProblem : GPProblem, ISimpleProblem
    {
        ...
        */
        ...
        // recursive trick to dump the full mesh into a bag.  Enter this by setting variable to 0,  Yuck, expensive.  But O(n).
        void buildIntervalPoints(IEvolutionState state, List<double[]> list, double[] min, double[] max, double[] interval, double[] current, int variable, int threadnum)
        {
            if (variable == min.Length)  // we're out of variables, base case
            {
                var d = new double[min.Length];
                for (var i = 0; i < d.Length; i++)
                    d[i] = current[i];      // not sure if System.arraycopy would be faster, probably not in this case
                list.Add(d);
            }
        ...
        public override Object Clone()
        {
            ...
            var myobj = (BenchmarksProblem)(base.Clone());
            ...
            if (problem == null)
            {
                ...
                else  // load from files
                {
                    try
                    {
                        ...
                        for (var i = 0; i < len; i++)
                        {
                            trainingInputs[i] = (double[])(input[i]);
                            trainingOutputs[i] = ((Double)(output[i]));
                        }
                        ...
                        for (int i = 0; i < len; i++)
                        {
                            testingInputs[i] = (double[])(input[i]);
                            testingOutputs[i] = ((Double)(output[i]));
                        }
                    }
