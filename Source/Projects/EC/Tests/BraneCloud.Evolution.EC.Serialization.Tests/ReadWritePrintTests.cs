...
namespace BraneCloud.Evolution.EC.Serialization.Tests
{
    ...
    public class ReadWritePrintTests
    {
        ...
        [TestMethod]
        [Description("GeneVectorIndividual's vital data is written to a binary stream and read back into another instance.")]
        public void GeneVectorIndividualWriteAndRead()
        {
            // First we'll set up a Fitness for the Individual
            var rand = new MersenneTwisterFast(0);
            var f = new SimpleFitness();
            ...
            var ind2 = new GeneVectorIndividual(); // We'll read back into this instance

            // Need to set the species so cloning will work properly
            var species = new GeneVectorSpecies();
            species.GenePrototype = new TestVectorGene();
            ind.Species = species;
            ind2.Species = species;

            ind.Genome = new VectorGene[10]; // This is the set of genes
            for (var i = 0; i < 10; i++)
            {
                ind.genome[i] = new TestVectorGene(); // Create a new gene with a random int value as its "Data"
            }
            ind.Fitness = f;
            ind.Evaluated = true;
        ...
        /// <summary>
        /// A minimally defined VectorGene derivation that has a single Int32 as it's data.
        /// NOTE: This is obviously for testing purposes only!
        /// </summary>
        [Serializable]
        private class TestVectorGene : VectorGene
        {
            private static IMersenneTwister s_rand = new MersenneTwisterFast();
            public int Data { get; set; }

            public TestVectorGene()
            {
                Data = s_rand.NextInt();
            }
            public TestVectorGene(int data)
            {
                Data = data;
            }
            ...
            {
                return Data == ((TestVectorGene)other).Data;
            }
            public override void Reset(IEvolutionState state, int thread)
            {
                Data = s_rand.NextInt();
            }
            public override void WriteGene(IEvolutionState state, BinaryWriter writer)
            {
                writer.Write(Data);
            }
            public override void ReadGene(IEvolutionState state, BinaryReader reader)
            {
