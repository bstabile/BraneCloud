...
namespace BraneCloud.Evolution.EC.GP.Koza
{
    ...
    ///  
    /// <p/>RAMPED HALF-AND-HALF works by choosing a random integer <i>d</i> between minDepth and maxDepth, inclusive.  
    /// It then grows a tree of depth 1 to <i>d</i> inclusive.  (1-pickGrowProbability) of the time (by default, 0.5) 
    /// it grows a tree using the FULL method, which generates full trees of exactly depth <i>d</i>.  (pickGrowProbability) 
    /// of the time, it grows a tree using the GROW method, which may generate trees of any size between 1 and <i>d</i> inclusive.
    /// <p/>Actually, claiming to implement the Koza I/II approach is a bit of a fib -- Koza's original code is somewhat ad-hoc.  
    /// In the Koza approach, <i>d</i> is chosen in a kind of round-robin fashion rather than at random, if RAMPED HALF/HALF is used.  
    /// Also, for all three algorithms (RAMPED HALF/HALF, GROW, FULL), the algorithm will not generate a tree consisting of a 
    /// single terminal, unless forced to.
    /// <p/>This implementation instead follows lil-gp's approach, which is to choose <i>d</i> at random from between minDepth 
    /// and maxDepth, inclusive, and to allow trees consisting of single terminals.   
    /// <p/>Determining what various algorithms do is a little confusing, mostly because the source code for lil-gp and Koza don't 
    /// actually quite do what they claim.  The table below lists the depth values actually used (counting nodes, rather than edges, 
    /// for depth).  It's probably not what you had expected!
    /// <br/>
    /// <br/>
    /// <div align="center">
    ...
    /// <br/>
    /// <br/>
    /// The table cell is empty when that parameter is not defined by the system by default.  Koza II has two values each 
    /// because of a possible typo in the text -- while page 656 gives one maximum, page 671 gives another.  
    ...
    /// </summary>		
    [Serializable]
    [ECConfiguration("ec.gp.koza.HalfBuilder")]
    public class HalfBuilder : KozaBuilder
    {
        #region Constants

        public const string P_HALFBUILDER = "half";
        public const string P_PICKGROWPROBABILITY = "growp";
        ...
        {
            get { return GPKozaDefaults.ParamBase.Push(P_HALFBUILDER); }
        }
