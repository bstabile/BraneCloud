...
namespace BraneCloud.Evolution.EC.Vector
{
    ...
    /// </summary>
    [Serializable]
    [ECConfiguration("ec.vector.IntegerVectorIndividual")]
    public class IntegerVectorIndividual : VectorIndividual
    {
        #region Constants

        public const string P_INTEGERVECTORINDIVIDUAL = "int-vect-ind";
        ...
        public override void Setup(IEvolutionState state, IParameter paramBase)
        {
            ...
            if (!(Species is IntegerVectorSpecies))
                state.Output.Fatal("IntegerVectorIndividual requires an IntegerVectorSpecies", paramBase, def);
        ...
        {
            var sum = pieces.Sum(t => ((int[])t).Length);

            var runningsum = 0;
            var newgenome = new int[sum];
            foreach (var t in pieces)
            {
                Array.Copy((Array)t, 0, newgenome, runningsum, ((int[])t).Length);
                runningsum += ((int[])t).Length;
            }
            // set genome
            genome = newgenome;
        }
        ...
        /// </summary>
        /// <remarks>Notice that we bump to longs to avoid overflow errors.</remarks>
        ...
        /// </summary>
        public virtual int RandomValueFromClosedInterval(int min, int max, IMersenneTwister random)
        {
            if (max - min < 0)
            // we had an overflow
            {
        ...
        {
            if (!(otherInd is IntegerVectorIndividual))
                return base.DistanceTo(otherInd);  // will return infinity!

            var other = (IntegerVectorIndividual)otherInd;
        ...
        {
            var myobj = (IntegerVectorIndividual)base.Clone();

            // must clone the genome, the property setter performs the cast
