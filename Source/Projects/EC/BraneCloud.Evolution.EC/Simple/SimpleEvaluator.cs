...
namespace BraneCloud.Evolution.EC.Simple
{
    ...
    public class SimpleEvaluator : Evaluator
    {
        ...
        void Expand(IEvolutionState state)
        {
            ...
            {
                pop.Subpops[i].Individuals = new Individual[numTests * state.Population.Subpops[i].Individuals.Count];
                for (int j = 0; j < state.Population.Subpops[i].Individuals.Count; j++)
                {
                    for (int k = 0; k < numTests; k++)
                    {
                        pop.Subpops[i].Individuals.Add((Individual)state.Population.Subpops[i].Individuals[j].Clone());
                    }
                }
            }
        ...
        public override void EvaluatePopulation(IEvolutionState state)
        {
            ...
            {
                int[] numinds = new int[state.Population.Subpops.Count];
                int[] from = new int[numinds.Length];

                for (int i = 0; i < numinds.Length; i++)
                {
                    numinds[i] = state.Population.Subpops[i].Individuals.Count;
                    from[i] = 0;
                }
        ...
        int ComputeChunkSizeForSubpopulation(IEvolutionState state, int subpop, int threadnum)
        {
            ...
            int individualsPerThread = state.Population.Subpops[subpop].Individuals.Count / numThreads;  // integer division
            int slop = state.Population.Subpops[subpop].Individuals.Count - numThreads * individualsPerThread;
        ...
        public override string RunComplete(IEvolutionState state)
        {
            for (int x = 0; x < state.Population.Subpops.Count; x++)
                for (int y = 0; y < state.Population.Subpops[x].Individuals.Count; y++)
        ...
        public class SimpleEvaluatorThread : IThreadRunnable
        {
            ...
            public virtual void Run()
            {
                ...
                int[] numinds = new int[subpops.Count];
                int[] from = new int[subpops.Count];
                ...
                while (true)
                {
                    ...
                    lock (Evaluator._locks)
                    {
                        ...
                        if (Evaluator.subPopCounter >= subpops.Count) // all done
                            return;

                        // has everyone finished the jobs for this subpopulation?
                        if (Evaluator.individualCounter >= subpops[Evaluator.subPopCounter].Individuals.Count)  // try again, jump to next subpop
                        {
                    ...
                    if (count >= subpops[subpop].Individuals.Count - start)
                        count = subpops[subpop].Individuals.Count - start;
